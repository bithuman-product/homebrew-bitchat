...
`bitchat` turns your Mac into a real-time conversational assistant. Speak,
type, or both — it transcribes, thinks, and replies out loud. In **video
mode**, a small floating face in a circular window moves its lips in sync
with the bot's voice. You can interrupt mid-sentence and it cuts off
within ~50 ms.

**Everything runs locally.** No data leaves your machine. No API keys. No
cloud round-trip. Works offline once the models are cached.
...
|---|---|---|
| `text` | Typed chat in the terminal. Pipe-friendly: `echo "hi" \| bitchat text`. | ~2 GB |
...
```sh
bitchat                                  # voice — pure audio chat
bitchat text                             # text-only repl
bitchat video                            # video chat with the default agent
bitchat video --image ~/Desktop/me.jpg   # video chat with your face
```

## ✨ Video mode — the centerpiece

`bitchat video` opens a small floating circular avatar window. **Right-click
the avatar** to customize.
...
bitchat running for hours without spinning the fans or draining the
battery.
...
(audio + avatar both). Cmd-Q, Ctrl-C, or right-click → "Quit bitchat" all
shut down cleanly.
...
- **Truly local.** No API keys, no per-token billing, no audio leaving
  your Mac. If your laptop's offline, bitchat still works.
...
- **Voice cloning out of the box.** Drop in a 10-second clip and bitchat
  uses it as the bot's voice (voice mode).
...
bitchat ships as a Swift library too — `BitChatKit` — embeddable via
Swift Package Manager. Build a custom voice / video assistant in a few
lines:
...
Open an issue at
[bithuman-product/homebrew-bitchat/issues](https://github.com/bithuman-product/homebrew-bitchat/issues)
if you'd like access.

## Tips

- **Set `BITCHAT_VERBOSE=1`** in your shell if you want to see model-loading
  internals (tensor counts, dtype breakdowns) while debugging. Silent by
  default.
- **First launch downloads ~3–7 GB of models** depending on mode. Plan
  for it on a slow connection — the rest of bitchat then works offline.
...
source code lives in a separate dev repo and is currently
invitation-only — open an issue here if you'd like access.

## Agents and LLMs

This repo publishes [`llms.txt`](llms.txt) — a structured manifest aimed
at AI coding assistants discovering and using bitchat. Agents trying to
install or invoke bitchat on a user's machine should start there.
